var roleEnergyCourier = {
    ...
    run: function(creep) {
	    ...
	    if (creep.memory.refueling) { // RETRIEVE ENERGY FROM CONTAINERS OR DROPPED ENERGY
            ...
            var spawn_targets = creep.room.find(FIND_STRUCTURES, {
                filter: (structure) => {
            ...
            var tower_targets = creep.room.find(FIND_STRUCTURES, {
                filter: (structure) => {
            ...
            if (spawn_targets.length > 0) {
                if(creep.transfer(spawn_targets[0], RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                    creep.moveTo(spawn_targets[0], {visualizePathStyle: {stroke: '#ffffff'}});
                }
            }
            else if (tower_targets.length > 0) {
                if(creep.transfer(tower_targets[0], RESOURCE_ENERGY) == ERR_NOT_IN_RANGE) {
                    creep.moveTo(tower_targets[0], {visualizePathStyle: {stroke: '#ffffff'}});
                }
            }
            else {
